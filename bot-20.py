...
def _ensure_users_table():
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS bot_users (user_id INTEGER PRIMARY KEY, first_seen TEXT)"
    )
    conn.commit()
    conn.close()


def _track_user(user_id):
    conn = sqlite3.connect(config.DB_PATH)
    conn.execute(
...
async def cmd_addgroup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.add_group(chat.id, chat.title, link.invite_link, config.OWNER_ID)
...
async def cmd_addchannel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Bot ke private chat me bhejo: /addchannel <channel_id>
    Channel me /addgroup nahi chalta (anonymous posts), isliye owner
    yaha se, private chat me (jaha uski identity verify hoti hai),
    channel register karta hai."""
    if update.effective_chat.type != "private":
        await update.effective_message.reply_text("Ye command sirf bot ke private chat me use karein.")
        return
    if not context.args:
        await update.effective_message.reply_text(
            "Usage: /addchannel <channel_id>\n\n"
            "Channel ID nikalne ke liye: channel me koi bhi message forward karke @userinfobot ko bhejein, "
            "ya channel ko bot me admin banane ke baad @RawDataBot use karein.\n"
            "Channel ID kuch aisi dikhti hai: -1001234567890"
        )
        return
    try:
        channel_id = int(context.args[0])
    except ValueError:
        await update.effective_message.reply_text("❌ Channel ID sahi format me do, jaise: -1001234567890")
        return

    try:
        chat = await context.bot.get_chat(channel_id)
    except TelegramError as e:
        await update.effective_message.reply_text(f"❌ Channel nahi mila: {e}\nPehle bot ko us channel me admin banao.")
        return

    if chat.type != "channel":
        await update.effective_message.reply_text("❌ Ye ID channel ki nahi hai.")
        return

    try:
        link = await context.bot.create_chat_invite_link(channel_id, creates_join_request=True, name="subscription-link")
    except TelegramError as e:
        await update.effective_message.reply_text(f"Link nahi ban paya: {e}\nBot ko channel me 'Invite Users via Link' admin permission do.")
        return

    db.add_group(channel_id, chat.title, link.invite_link, config.OWNER_ID)
    await update.effective_message.reply_text(
        f"✅ Channel register ho gaya!\n\nChannel ID: `{channel_id}`\nTitle: {chat.title}\n"
        f"Invite Link (approval-required):\n{link.invite_link}\n\n"
        f"Ab is channel ke liye /addplan aur /grant use kar sakte ho.",
        parse_mode="Markdown",
    )
...
async def cmd_backup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner turant manually backup mangwa sakta hai: /backup"""
    try:
        with open(config.DB_PATH, "rb") as f:
            ...
            await update.effective_message.reply_document(
                ...
                caption=f"🗄️ Manual Backup — {timestamp}",
            )
    except FileNotFoundError:
        await update.effective_message.reply_text("❌ DB file nahi mili.")
    except TelegramError as e:
        await update.effective_message.reply_text(f"❌ Backup bhejne me error: {e}")


@owner_only
async def cmd_restore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Backup .db file ko reply karke: /restore
    ⚠️ Current database ko overwrite kar deta hai — sambhal ke use karein."""
    if update.effective_chat.type != "private":
        await update.effective_message.reply_text("Ye command sirf bot ke private chat me use karein.")
        return
    ...
    if not source or not source.document:
        await msg.reply_text(
            "Jo backup `.db` file restore karni hai, usko *reply* karke /restore likhein.\n\n"
            "⚠️ *Warning:* Isse current database poori tarah overwrite ho jayegi. "
            "Restore se pehle current DB ka backup zaroor le lein (/backup).",
            parse_mode="Markdown",
        )
        return

    if not source.document.file_name.endswith(".db"):
        await msg.reply_text("❌ Ye `.db` file nahi lag rahi. Sirf backup `.db` file hi restore karein.")
        return

    if not context.args or context.args[0].lower() != "confirm":
        await msg.reply_text(
            f"⚠️ *Pakka restore karna hai?*\n\n"
            f"File: `{source.document.file_name}`\n"
            f"Size: {source.document.file_size} bytes\n\n"
            f"Ye current database ko *permanently overwrite* kar dega. "
            f"Confirm karne ke liye isi message ko reply karke likhein: `/restore confirm`",
            parse_mode="Markdown",
        )
        return
    ...
    try:
        ...
        await tg_file.download_to_drive(config.DB_PATH)
        await msg.reply_text(
            "✅ Database restore ho gaya!\n\n"
            "⚠️ Ab bot ko *restart* karein taaki naya data properly load ho jaaye.",
            parse_mode="Markdown",
        )
        log.info(f"Database restored from {source.document.file_name} by owner")
    except TelegramError as e:
        await msg.reply_text(f"❌ Restore me error: {e}")


@owner_only
async def cmd_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner ke private chat me:
    - Kisi bhi message (text/photo/sticker/premium emoji, kuch bhi) ko REPLY karke /broadcast likho
    - Ya sirf /broadcast <text> bhi likh sakte ho
    Sabhi tracked users ko wahi message copy ho jaata hai (jaisa ka taisa, formatting/emoji sahit)."""
    if update.effective_chat.type != "private":
        await update.effective_message.reply_text("Ye command sirf bot ke private chat me use karein.")
        return
    ...
    if source is None:
        if not context.args:
            await msg.reply_text(
                "Jo message broadcast karna hai usko *reply* karke /broadcast likhein "
                "(text, photo, sticker, premium emoji — kuch bhi chalega).\n\n"
                "Ya sirf: /broadcast <text>",
                parse_mode="Markdown",
            )
            return
    ...
    if not users:
        await msg.reply_text("❌ Abhi tak koi user track nahi hua.")
        return

    status = await msg.reply_text(f"📢 Broadcast shuru... 0/{len(users)}")
    sent, failed = 0, 0
    for i, user_id in enumerate(users, start=1):
        ...
        if i % 25 == 0:
            try:
                await status.edit_text(f"📢 Broadcast ja raha hai... {i}/{len(users)}")
            except TelegramError:
                pass
...
async def backup_job(application):
    """DB file ko roz owner ko Telegram document ke roop me bhej deta hai"""
    try:
        with open(config.DB_PATH, "rb") as f:
            timestamp = datetime.now().strftime("%d-%b-%Y_%H-%M")
            await application.bot.send_document(
                ...
                document=f,
                filename=f"backup_{timestamp}.db",
                caption=f"🗄️ Daily Backup — {timestamp}",
            )
        log.info("Daily backup sent to owner")
    except FileNotFoundError:
        log.warning(f"backup failed: DB file not found at {config.DB_PATH}")
    except TelegramError as e:
        log.warning(f"backup send failed: {e}")


def setup_scheduler(application):
    scheduler = AsyncIOScheduler()
    scheduler.add_job(check_expired_job, "interval", minutes=config.CHECK_INTERVAL_MINUTES, args=[application])
    scheduler.add_job(backup_job, "cron", hour=3, minute=0, args=[application])
    scheduler.start()
    return scheduler
