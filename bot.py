...
def owner_only(func):
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Channel posts anonymous hote hain (effective_user None), lekin sirf
        # OWNER hi channel me admin hokar post kar sakta hai, isliye allow karo.
        if update.effective_chat and update.effective_chat.type == "channel":
            pass
        elif not user or user.id != config.OWNER_ID:
...
async def cmd_addgroup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Group/Channel ke andar bhejo: /addgroup"""
    chat = update.effective_chat
    if chat.type not in ("group", "supergroup", "channel"):
        await update.effective_message.reply_text("Ye command group ya channel ke andar bhejo, jaha bot admin ho.")
        return
    try:
    ...
    await update.effective_message.reply_text(
        f"✅ Group/Channel register ho gaya!\n\nID: `{chat.id}`\nInvite Link (approval-required):\n{link.invite_link}\n\n"
        f"Ab is group/channel ke liye /addplan aur /grant use kar sakte ho.",
        parse_mode="Markdown",
    )
