...
async def cmd_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner ke private chat me: /broadcast <message>
    Sabhi users ko jinhone kabhi /start kiya hai, message bhej deta hai."""
    if update.effective_chat.type != "private":
        await update.effective_message.reply_text("Ye command sirf bot ke private chat me use karein.")
        return
    if not context.args:
        await update.effective_message.reply_text(
            "Usage: /broadcast <message>\n\nMarkdown formatting support hai (*bold*, _italic_, etc.)"
        )
        return
    ...
    if not users:
        await update.effective_message.reply_text("❌ Abhi tak koi user track nahi hua.")
        return

    status = await update.effective_message.reply_text(f"📢 Broadcast shuru... 0/{len(users)}")
    sent, failed = 0, 0
